...
void Heap<T>::insert(T value) {
  // add the value to the bottom of the vector
  values.push_back(value);
  // store the current index of the new value
  int index_value = values.size() - 1;
  // check if parent needs to be heapified
  int parent_index = floor((index_value-1)/2);
...
void Heap<T>::remove(T value) {
  ...
  if (index != -1){
    // replace the value at this index with the right most value in the bottom most (last value) in the heap
    values[index] = values[values.size()-1];

    // remove the right most value in the bottom most (last value) in the heap
    values.pop_back();

    // keep heapifying till the heap doesn't chage
    std::vector<int> current_values;

    while (current_values != values){
